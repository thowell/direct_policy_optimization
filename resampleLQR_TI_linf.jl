...
n_nlp = m*n*(T-1) + N*(n*(T-1) + m*(T-1)) + N*((T-1) + (T-1))
...
idx_sx = [[(T-1)*(m*n) + N*(n*(T-1) + m*(T-1)) + (i-1)*((T-1) + (T-1)) + (t-1)*2 .+ (1:1) for t = 1:T-1] for i = 1:N]
idx_su = [[(T-1)*(m*n) + + N*(n*(T-1) + m*(T-1)) + (i-1)*((T-1) + (T-1)) + (t-1)*2 + 1 .+ (1:1) for t = 1:T-1] for i = 1:N]
...
function con!(c,z)
    ...
    for t = 1:T-1
        ...
        sx = [view(z,idx_sx[i][t])[1] for i = 1:N]
        su = [view(z,idx_su[i][t])[1] for i = 1:N]

        for i = 1:N
            c[idx_con_dyn[i][t]] = xs⁺[i] - x⁺[i]
            c[idx_con_ctrl[i][t]] = u[i] + k*xs[i]
            c[idx_con_sxp[i][t]] = sqrt(Q)*xs⁺[i] - sx[i]*ones(n)
            c[idx_con_sxn[i][t]] = -sqrt(Q)*xs⁺[i] - sx[i]*ones(n)
            c[idx_con_sup[i][t]] = sqrt(R)*u[i] - su[i]*ones(m)
            c[idx_con_sun[i][t]] = -sqrt(R)*u[i] - su[i]*ones(m)
        end
    end
    return c
end
