function general_constraints!(c,Z,prob::TrajectoryOptimizationProblem)
	...
	c[1:nx] = legendre(model,x1,x2,x3,u1,h1) - prob.v1
end
...
function ∇general_constraints!(∇c,Z,prob::TrajectoryOptimizationProblem)
	...
	lx1(y) = legendre(model,y,x2,x3,u1,h1)
	lx2(y) = legendre(model,x1,y,x3,u1,h1)
	lx3(y) = legendre(model,x1,x2,y,u1,h1)
	lu1(y) = legendre(model,x1,x2,x3,y,h1)
	lh1(y) = legendre(model,x1,x2,x3,u1,y)
	...
	∇c[shift .+ (1:len)] = vec(ForwardDiff.jacobian(lx1,x1))
	shift += len
