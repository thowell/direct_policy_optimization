...
eval_objective(prob_sample,Z0_sample)
...
out = zeros(prob_sample.M_nlp,prob_sample.N_nlp)
FiniteDiff.finite_difference_jacobian!(out,tmp_c,Z0_sample)

@assert norm(vec(∇c) - vec(out),Inf) < 1.0e-6
@assert sum(∇c) - sum(out) < 1.0e-6
