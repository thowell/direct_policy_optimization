function general_constraints!(c,Z,prob::TrajectoryOptimizationProblem)
	nx = prob.nx
	T = prob.T
	c[1:nx] = Z[prob.idx.x[1]] - Δ(Z[prob.idx.x[T]])
	nothing
end
...
function ∇general_constraints!(∇c,Z,prob::TrajectoryOptimizationProblem)
	...
	c_idx = prob.idx.x[T]
	len = length(r_idx)*length(c_idx)
	∇c[s .+ (1:len)] = vec(-1.0*ForwardDiff.jacobian(Δ,Z[prob.idx.x[T]]))
	s += len
...
# 	nx = prob.prob.nx
# 	for i = 1:N
# 		c[(i-1)*nx .+ (1:nx)] = Z[prob.idx_sample[i].x[1]] - Δ(Z[prob.idx_sample[i].x[T]])
# 	end
# 	nothing
# end
#
...
# 		len = length(r_idx)*length(c_idx)
# 		∇c[s .+ (1:len)] = vec(Diagonal(-1.0*ForwardDiff.jacobian(Δ,Z[prob.idx_sample[i].x[T]])))
# 		s += len
# 	end
# 	nothing
# end
