...
desired = Point3D(world,-0.25,2.1,exp(model.a_exp*(0.0-model.shift_exp)))
...
desired = Point3D(world,0.25,1.9,exp(model.a_exp*(0.0-model.shift_exp)))
...
@time Z_nominal = solve(prob_moi,copy(Z0),nlp=:ipopt,
	c_tol=1.0e-2,tol=1.0e-2,max_iter=500,time_limit=60*5)
