...
θ_sol = vec([x_sol[t][1] for t = 1:T])
dθ_sol = vec([x_sol[t][2] for t = 1:T])
...
R = [Matrix((0.001)*I,m,m) for t = 1:T-1]
...
for t = 1:T-1
    ...
    for i = 1:N
        ...
        z_sol_K[idx_x[i][t]] = A[t]*(t==1 ? x1[i] : z_sol_K[idx_x[i][t-1]]) + B[t]*z_sol_K[idx_u[i][t]]
    end
end
...
T_sim = 10*T
μ = zeros(2)
Σ = Diagonal(1.0e-12*rand(2))
W = Distributions.MvNormal(μ,Σ)
w = rand(W,T_sim)
z0_sim = [0.0;0.0]

plt = plot(θ_sol,dθ_sol,xlabel="θ",ylabel="dθ",width=2.0,color=:red)
z_tvlqr, u_tvlqr = simulate_linear_controller(K,x_sol,u_sol,T_sim,Δt,z0_sim,w)
x_tvlqr = [z_tvlqr[t][1] for t = 1:T_sim]
y_tvlqr = [z_tvlqr[t][2] for t = 1:T_sim]
plt = plot!(x_tvlqr,y_tvlqr,color=:purple,label="",width=2.0)

z_sample, u_sample = simulate_linear_controller(K_sample,x_sol,u_sol,T_sim,Δt,z0_sim,w)
