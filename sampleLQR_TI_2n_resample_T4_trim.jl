...
n_nlp = N*(n*(2) + m*(2)) + m*n*(T-1) + N*(n*(3) + m*(2))# + N*(n*(2) + m*(1))
m_nlp = N*(n*(2)) + N*(m*(2)) + N*(n*(3)) + N*(m*(2))# + N*(n*(2)) + N*(m*(1))
...
function obj(z)
    ...
    return (u11'*R*u11 + u12'*R*u12 + u13'*R*u13 + u14'*R*u14
            ...
            # + û31'*R*û31 + û32'*R*û32 + û33'*R*û33 + û34'*R*û34
            # + x̂31'*Q*x̂31 + x̂32'*Q*x̂32 + x̂33'*Q*x̂33 + x̂34'*Q*x̂34
            # + x̂41'*Q*x̂41 + x̂42'*Q*x̂42 + x̂43'*Q*x̂43 + x̂44'*Q*x̂44)
...
function con!(c,z)
    ...
    ##
    c[45:46] = A*x̃31 + B*ũ31 - x̃41
    c[47:48] = A*x̃32 + B*ũ32 - x̃42
    c[49:50] = A*x̃33 + B*ũ33 - x̃43
    c[51:52] = A*x̃34 + B*ũ34 - x̃44
    ...
    #
    # # resampled constraints
    # Σ_ = x̃31*x̃31' + x̃32*x̃32' + x̃33*x̃33' + x̃34*x̃34' + 1.0e-8*I
    # cols_ = Array(cholesky(Σ_).U)
    # β = 0.0001
    #
    # c[57:58] = β*cols_[:,1] - x̂31
    # c[59:60] = β*cols_[:,1] - x̂32
    # c[61:62] = β*cols_[:,2] - x̂33
    # c[63:64] = β*cols_[:,2] - x̂34
    #
    # c[65:66] = A*x̂31 + B*û31 - x̂41
    # c[67:68] = A*x̂32 + B*û32 - x̂42
    # c[69:70] = A*x̂33 + B*û33 - x̂43
    # c[71:72] = A*x̂34 + B*û34 - x̂44
    #
    # c[73] = û31 + k3'*x̂31
    # c[74] = û32 + k3'*x̂32
    # c[75] = û33 + k3'*x̂33
...
K_sample = [reshape(z_sol[5:6],m,n), reshape(z_sol[19:20],m,n),reshape(z_sol[49:50],m,n)]
