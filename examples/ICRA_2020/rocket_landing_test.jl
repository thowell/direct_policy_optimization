...
x1_slosh = [5.0; model.l2+10.0; -10.0*pi/180.0; 0.0*pi/180.0; -5.0; -5.0; -1.0*pi/180.0; 0.0*pi/180.0]
...
# DPO
N = 2*model_slosh.nx
# mf = range(0.5*model_slosh.mf,stop=1.5*model_slosh.mf,length=N)
# mr = [1.0-mf[i] for i = 1:N]
# lf = shuffle(range(0.5*model_slosh.l3,stop=1.5*model_slosh.l3,length=N))
...
# ψ = shuffle(range(-pi/10,stop=pi/10,length=N))
...
#
# Z0_sample2 = pack(X_nom_sample,X_sample[1],U_nom_sample,H_nom_sample[1],[vec(K[t]) for t = 1:T-1],prob_sample2)
