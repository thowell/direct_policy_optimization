...
xT_nom = [1.0; 2.0; 0.0; 0.0; 0.0; 0.0]
...
# Sample
γ = 1.0
Q_lqr = [γ*Diagonal(1.0*ones(model.nx)) for t = 1:T]
R_lqr = [γ*Diagonal(1.0*ones(nu)) for t = 1:T-1]

A_dyn, B_dyn = nominal_jacobians(model,X_nom,U_nom,Δt=Δt,free_time=false)
K = TVLQR([A_dyn[1] for t = 1:T-1],[B_dyn[1] for t = 1:T-1],Q_lqr,R_lqr)
...
plt = plot(policy_error_linear,xlabel="time step",ylabel="matrix-norm error",yaxis=:log,
    ylims=(1.0e-8,10.0),width=2.0,legend=:bottom,label="")
