...
Q_nom = [t < T ? Diagonal([1.0; 1.0; 10.0; 10.0]) : Diagonal([1.0; 1.0; 10.0; 10.0]) for t = 1:T]
R_nom = [Diagonal(1.0*ones(model.nu)) for t = 1:T-1]
c_nom = 0.0
...
# Sample
Q_lqr = [t < T ? Diagonal([10.0;10.0;1.0;1.0]) : Diagonal([100.0;100.0;100.0;100.0]) for t = 1:T]
...
α = 1.0
x11 = α*[1.0; 0.0; 0.0; 0.0]
x12 = α*[-1.0; 0.0; 0.0; 0.0]
x13 = α*[0.0; 1.0; 0.0; 0.0]
x14 = α*[0.0; -1.0; 0.0; 0.0]
x15 = α*[0.0; 0.0; 1.0; 0.0]
x16 = α*[0.0; 0.0; -1.0; 0.0]
x17 = α*[0.0; 0.0; 0.0; 1.0]
x18 = α*[0.0; 0.0; 0.0; -1.0]

x1_sample = [x11,x12,x13,x14,x15,x16,x17,x18]
...
# linear dynamics
function discrete_dynamics(model::Acrobot,x⁺,x,u,h,t)
...
savefig(plt,joinpath(@__DIR__,"results/TVLQR_acrobot.png"))
...
PGF.save(joinpath(dir,"TVLQR_acrobot.tikz"), a, include_preamble=false)
