...
K0 = [rand(model.nu_ctrl*(model.nx*2 + model.nc*2)) for t = 1:T-2]
β = 1.0
...
prob_sample = init_sample_problem(prob,models,x1_sample,
	...
	nK=model.nu_ctrl*(model.nx*2 + model.nc*2),
    β=β,w=w,γ=γ,
...
display(pltz)
savefig(pltz,joinpath(@__DIR__,"results/particle_contact_feedback_z_T$T.png"))
