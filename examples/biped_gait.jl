...
@time Z_nominal_step = solve(prob_moi,copy(Z0),nlp=:SNOPT,max_iter=100,time_limit=30)
...
# Trajectory initialization
X0 = x_gait_ref # linear interpolation on state
norm(X0[1] - x1_gait)
norm(X0[Tm][1:5] - xT[1:5])
...
# Solve nominal problem
@time Z_nominal = solve(prob_moi,copy(Z0),nlp=:SNOPT7,max_iter=200,time_limit=20)
...
z_tvlqr, u_tvlqr, J_tvlqr, Jx_tvlqr, Ju_tvlqr = simulate_linear_controller(K_lqr,
    ...
    ul=ul,uu=uu)
plt = plot!(t_sim_nominal,hcat(z_tvlqr...)[1:5,:]',color=:purple,
    label=["tvlqr" "" "" "" ""],width=2.0,legend=:left)

plt = plot(t_sample,hcat(X_nom_sample...)[1:5,:]',color=:red,label="",
    width=2.0,xlabel="time (s)",title="Biped",ylabel="state",legend=:left)
